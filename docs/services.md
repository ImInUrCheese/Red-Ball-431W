...
listing_service        → image_service
                       → bid_service (inline import to avoid circular import)
helpdesk_service       (no dependencies)
rating_service         (no dependencies)
```

**Circular import note:** `listing_service` imports from `bid_service` inside function bodies (not at the module top level) to avoid a circular import. This pattern should be followed if other cross-service calls are needed between these two files.
...
| Function | Parameters | Returns | Description |
|---|---|---|---|
| `get_bidder_profile` | `email` | `dict \| None` | Returns `email`, `first_name`, `last_name`, `age`, `major`, `home_address_id`, `image_url`. `None` if not found |
| `get_seller_profile` | `email` | `dict \| None` | Returns `email`, `bank_routing_number`, `bank_account_number`, `balance`, `image_url` |
| `get_helpdesk_profile` | `email` | `dict \| None` | Returns `email`, `position`, `image_url` |
| `get_payment_info` | `email` | `dict \| None` | Returns `card_type`, `last_four`, `expire_month`, `expire_year` for the bidder's first card. `None` if no card on file |

All profile read functions include an `image_url` field resolved via `image_service.get_image_url`.
...
| `upload_user_image` | `email`, `file` | `dict` | Validates and saves a profile image. Returns `{success, image_url}` or `{success, error}` |
...
| `get_leaf_categories` | — | `list[str]` | Returns all category names that have no children — these are the only valid categories for a listing |
| `get_listings_by_category` | `category_name` | `list[dict]` | Returns all active (`status=1`) listings under a category, each with live bid stats merged in |
...
| Function | Parameters | Returns | Description |
|---|---|---|---|
| `get_seller_listings` | `seller_email` | `dict` | Returns all listings grouped by status: `{active: [...], inactive: [...], sold: [...]}` |
| `get_seller_active_listings` | `seller_email` | `list[dict]` | Returns only active listings for a seller, each with live `highest_bid`, `bid_count`, `bids_remaining` |
| `get_seller_sales_history` | `seller_email` | `list[dict]` | Returns ended listings (status 0 or 2) with `final_payment` and `date` for sold items |
| `create_listing` | `seller_email`, `category`, `auction_title`, `product_name`, `product_description`, `quantity`, `reserve_price`, `max_bids` | `dict` | Creates a new active listing. Auto-assigns the next `listing_id` for that seller. Returns `{success, listing_id}` |
| `update_listing` | `seller_email`, `listing_id`, `**fields` | `dict` | Updates allowed fields. Blocked if listing is sold (`status=2`) or active with existing bids |
...
| `upload_listing_image` | `seller_email`, `listing_id`, `file` | `dict` | Validates and saves a listing image. Returns `{success, image_url}` or `{success, error}` |

**`update_listing` allowed fields:** `category`, `auction_title`, `product_name`, `product_description`, `quantity`, `reserve_price`, `max_bids`

All listing serializers include an `image_url` field resolved via `image_service.get_image_url`.

**`get_seller_sales_history` response shape per item:**
`listing_id`, `auction_title`, `category`, `final_payment` (null if unsold), `date` (null if unsold), `status` (`"sold"` or `"unsold"`)

### Search

| Function | Parameters | Returns | Description |
|---|---|---|---|
| `search_listings` | `keyword?`, `min_price?`, `max_price?` | `list[dict]` | Searches active listings using SQL `ILIKE` across title, name, description, category, and seller email. Each result includes live bid stats |

---

## bid_service.py

Handles bid placement, bid queries, and auction completion logic. Calls `notification_service` internally when an auction ends.

| Function | Parameters | Returns | Description |
|---|---|---|---|
| `place_bid` | `bidder_email`, `seller_email`, `listing_id`, `bid_price` | `dict` | Places a bid after validating all three rules. Triggers `_close_auction` if `max_bids` is reached. Returns `{success, highest_bid, bids_remaining, auction_complete}` or `{success, error}` |
| `get_listing_bids` | `seller_email`, `listing_id` | `dict \| None` | Returns `bid_count`, `bids_remaining`, `highest_bid`, `highest_bidder`. `None` if listing not found |
| `get_bid_history` | `seller_email`, `listing_id` | `list \| None` | Returns `[{bidder_email, bid_price}]` ordered by price descending. `None` if listing not found |
| `get_bids_by_bidder` | `bidder_email` | `list[dict]` | Returns all active auctions the bidder has bid on, deduplicated to highest bid per listing. Each item includes `seller_email`, `listing_id`, `auction_title`, `your_bid`, `highest_bid`, `bids_remaining`, `leading` |
| `check_auction_complete` | `seller_email`, `listing_id` | `dict \| None` | Returns `{complete: False}` or `{complete: True, winner, reserve_met, highest_bid}` |

**Bid validation rules enforced by `place_bid`:**

| Rule | Error returned |
|---|---|
| Auction end rule | `"Auction has ended"` — bid count must be less than `max_bids` |
| Increment rule | `"Bid must be at least $X.XX"` — new bid must be at least $1.00 above current highest |
| Turn-taking rule | `"You must wait for another bidder before bidding again"` — a bidder cannot place two consecutive bids |

**Auction completion (`_close_auction`, internal):**

Called automatically by `place_bid` when `max_bids` is reached.

- **Reserve met:** Sets `status=0`, sends `auction_won` + `payment_due` to winner, `auction_lost` to all other bidders
- **Reserve not met:** Sets `status=0`, sends `auction_ended_no_sale` to all bidders
...
**Request types** (not exhaustive): `ChangeID`, `AddCategory`, `MarketAnalysis`
...
| `get_seller_average_rating` | `seller_email` | `float \| None` | Returns average rating rounded to 2 decimal places. `None` if no ratings |
